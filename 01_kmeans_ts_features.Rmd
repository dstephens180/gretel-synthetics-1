...
## Save for wide & long synthetic data
```{r}
cluster_row_number <- 
  cluster_id_connected %>%
  distinct(cluster, .keep_all = F) %>%
  mutate(cluster_number = row_number())



synthetic_input_long <- 
  cluster_id_connected %>%
  left_join(date_value_tbl, by = "id") %>%
  mutate(id = str_c("listing_id_", id)) %>%
  group_by(id) %>%
  pad_by_time(
    date,
    .by = "day", 
    .pad_value = NA,
    .start_date = min(date_value_tbl$date), 
    .end_date = max(date_value_tbl$date)
  ) %>%
  mutate(value = ts_impute_vec(value, period = 365, lambda = NULL)) %>%
...
synthetic_input_long %>% 
  filter(id == "listing_id_737") %>%
  plot_time_series(
    date, value, .color_var = id, .smooth = F
  )

write_csv(synthetic_input_long, "00_data/track_combined_calendar_synthetic_long_svr.csv")
write_csv(cluster_row_number, "00_data/cluster_row_number_svr.csv")
